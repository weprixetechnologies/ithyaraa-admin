...
import { getHomepageSectionByID, updateHomepageSection, searchCategories, searchOffers } from '../../lib/api/homepageSectionsApi';
...
const EditHomepageSection = () => {
        ...
        const fetchSection = async () => {
            try {
                ...
                if (res.success && res.data) {
                    const data = res.data;
                    // Remove leading slash from routeTo if present
                    const routeTo = data.routeTo && data.routeTo.startsWith('/')
                        ? data.routeTo.slice(1)
                        : (data.routeTo || 'shop');
    ...
    const handleSubmit = async () => {
        try {
            ...
            const imageUrl = imageUpload && imageUpload.length > 0 ? imageUpload[0].imgUrl : sectionData.image;
            ...
            if (res.success) {
                ...
                toast.error(res.error || res.message || 'Failed to update section');
            }
        } catch (err) {
    ...
    return (
        <Layout title="Edit Homepage Section" active="admin-homepage-sections-list">
            <div className="grid grid-cols-6 gap-5">
                ...
                <div className="col-span-2 gap-2">
                    <div className="flex flex-col gap-2">
                        <Container label="Section Image">
                            <UploadImages
                                ref={imageRef}
                                defaultImages={sectionData.image ? [sectionData.image] : []}
                                maxImages={1}
                            />
                        </Container>
                        <button
