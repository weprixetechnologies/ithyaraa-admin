...
import { orderDetailsArray } from '../ordersSchema';
...
const OrderList = () => {
    ...
    const [orders, setOrders] = useState(orderDetailsArray);

    useEffect(() => {
        let filtered = [...orderDetailsArray];
        ...
        if (filters.orderId) {
            filtered = filtered.filter(order => order.ordersId.includes(filters.orderId));
        }
        ...
        if (filters.date && filters.dateType) {
    ...
    return (
        <Layout active={`admin-orders-${status}`} title={`Order List - ${status.toUpperCase()}`}>
            <div className="" style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>

                <Container title={'Filter Orders'} gap={10}>
